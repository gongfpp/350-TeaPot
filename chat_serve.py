...
# 添加用户函数
def add_user(user_name, password, file_name,file_reco_name):
    # 调用数据库模块中的函数添加用户，成功返回0，已有用户返回1，其他错误返回2
    if chat_mysql.LogInformation.select_user_name(user_name) == "1":
        return "1"
    elif chat_mysql.LogInformation.create_new_user(user_name, password, file_name,file_reco_name) == "0":
        return "0"
    else:
        return "2"
...
def handle_register(connection, address):
    ...
    file_reco_name=recv_all_string(connection)
    # 调用添加用户函数add_user返回值作为发送给客户端的标记，0成功，1已有用户，2其他错误
    connection.sendall(bytes(add_user(user_name, password, file_name,file_reco_name), "utf-8"))
    return True
