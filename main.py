...
from numpy import take   # 导入文件选择对话框模块
# 导入自定义模块
...
def handding_register():
    ...
    register_frame = chat_register_panel.RegisterPanel(file_open_face, close_register_window, register_submit,register_photo)
    # 调用对象的实例方法显示注册界面
...
def close_register_window():
    ...
    # 可以把chat_login_pannel模块的事件绑定在这几个函数上
    login_frame = chat_login_panel.LoginPanel(handding_login, handding_register, close_login_window,handding_faceing)
...
def register_submit(self):
    ...
    user_name, password, confirm_password, file_name,file_reco_name = register_frame.get_input()
    # 判断用户名，密码，确认密码是否为空
    ...
    if register_frame.file_name == "":
        ...
        return
    # 对象调用实例方法发送消息给服务器
    result = client.register_user(user_name, password, file_name,file_reco_name)
    if result == "0":
        # 注册成功，跳往登陆界面
...
#注册时人脸拍照函数        
def register_photo():
    messagebox.showwarning(title="提示", message="开始拍照,按空格退出")
    register_frame.takep()
