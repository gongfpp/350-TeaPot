...
class RegisterPanel(object):
    ...
    def __init__(self, file_open_face, close_register_window, register_submit,register_photo):
        # 初始化参数实例变量
        self.file_open_face = file_open_face
        self.close_register_window = close_register_window
        self.register_submit = register_submit
        self.file_name = ""  # 文件路径
        self.file_reco_name = ""  #人脸文件路径
        self.register_photo=register_photo
    ...
    def show_register_panel(self):
        ...
        #!!!!
        #self.p1 = PhotoImage(file='pictures/添加按钮.png')  # 把图片转化为PhotoImage类型

        numIdx = 3  # gif的帧数
        # 循环遍历动图的帧
        frames = [PhotoImage(file='pictures/login.gif', format='gif -index %i' % (i)) for i in range(numIdx)]
        # 创建存放gif的标签
        ...
        # 设置文本框的位置
        self.face_show.place(x=370, y=210)

        # 声明宽度高度，用来设置图片大小
        self.width = 50
        self.height = 50
        # 打开图片，用在注册页面文本框中显示默认头像
        img = Image.open("pictures/默认头像.png")
        # 设置图片的大小
        out = img.resize((self.width, self.height), Image.ANTIALIAS)
        # 保存图片，类型为png
        out.save(r"regisPic/头像.png", 'png')

        # 把头像转换为PhotoImage类型，用于在文本框显示
        self.p2 = PhotoImage(file='regisPic/头像.png')
        # 设置文本框可编辑
        ...
        #-----人脸框----
        # 设置文本框，用户存放头像
        self.reco_show = Text(self.register_frame, bg="white", height=3.5, width=7,
                                 highlightcolor="white")
        # 设置文本框不可编辑
        self.reco_show.config(state=DISABLED)
        # 设置文本框的位置
        self.reco_show.place(x=370, y=270)

        # 打开图片，用在注册页面文本框中显示默认头像
        img2 = Image.open("pictures/wx.png")
        # 设置图片的大小
        out2 = img2.resize((self.width, self.height), Image.ANTIALIAS)
        # 保存图片，类型为png
        out2.save(r"regisPic/人脸.png", 'png')

        # 把头像转换为PhotoImage类型，用于在文本框显示
        self.p3 = PhotoImage(file='regisPic/人脸.png')
        # 设置文本框可编辑
        self.reco_show.config(state=NORMAL)
        # 把头像图片插入文本框
        self.reco_show.image_create(END, image=self.p3)
        # 设置文本框不可编辑
        self.reco_show.config(state=DISABLED)
        # 设置文本框滑到最低
        self.reco_show.see(END)
        ...
        self.botton_file_open = Button(self.register_frame, text="上传头像",relief=FLAT,bg='grey', fg="blue",
                                       command=self.file_open_face).place(x=430, y=230)
        
        # 设置添加人脸识别及位置，事件处理为为register_photo函数
        self.botton_file_open = Button(self.register_frame, text="添加人脸",relief=FLAT,bg='grey', fg="red",
                                       command=self.register_photo).place(x=430, y=290)
    
    
    def takep(self):
        print('\n 数据初始化中，请直视摄像机录入数据....')
        vc = cv2.VideoCapture(0)
        face_id=self.user_name.get()
        while True:
            # 获取视频中每一帧的图片；
            ret, img = vc.read()
            # 3 发现人脸图片中人脸的位置
            ...
            if cv2.waitKey(1) == 32:
                for i in range(1,2):
                # 保存图像
                  cv2.imwrite('peopleface/' + str(face_id) + '.png',img)
        ...
        self.file_reco_name='peopleface/' + str(face_id) + '.png'
        # 打开图片
        img = Image.open('peopleface/' + str(face_id) + '.png')
        # 设置图片大小
        out = img.resize((self.width, self.height), Image.ANTIALIAS)
        # 保存图片，类型为png
        out.save(r"regisPic/人脸.png", 'png')
        # 把头像转化为PhotoImage
        self.p4 = PhotoImage(file='regisPic/人脸.png')
        # 设置文本框可编辑
        self.reco_show.config(state=NORMAL)
        self.reco_show.delete('0.0', END)
        # 把头像插入文本框
        self.reco_show.image_create(END, image=self.p4)
        # 设置文本不可编辑
        self.reco_show.config(state=DISABLED)
        # 设置文本框滑到最低
        self.reco_show.see(END)
    ...
    def add_face(self, file_name):
        ...
        out.save(r"regisPic/头像.png", 'png')
        # 把头像转化为PhotoImage
        self.p = PhotoImage(file='regisPic/头像.png')
        # 设置文本框可编辑
        self.face_show.config(state=NORMAL)
    ...
    # 获取输入的用户名、密码、确认密码、头像、人脸实例方法
    def get_input(self):
        return self.user_name.get(), self.password.get(), self.confirm_password.get(), self.file_name,self.file_reco_name
