...
class LogInformation(object):
    ...
    def login_check(user_name, password):  # 检查用户登录
        db = pymysql.connect(host="121.37.186.99", user="public", password="test", db="python_chat")  # 关联数据库
    ...
    @staticmethod  # 静态函数方便调用
    def create_new_user(user_name, password, file_name,file_reco_name):  # 创建新用户
        db = pymysql.connect(host="121.37.186.99", user="public", password="test", db="python_chat")  # 关联数据库
        cursor = db.cursor()  # 取得数据库游标
        fp = open(file_name, 'rb')  # 打开头像路径
        img = fp.read()  # 读取头像
        print(type(img))
        fp = open(file_reco_name, 'rb')  # 打开头像路径
        face = fp.read()  # 读取人脸
        print(type(face))
        sql = "INSERT INTO user_information VALUES  (%s,%s,%s,%s);"  # 数据库语言，插入用户数据
        args = (user_name, password, img,face)
        try:
    ...
    def fing_face(user_name):  # 查找头像，用于聊天界面显示头像
        try:
            conn = pymysql.connect(host="121.37.186.99", user="public", password="test", db="python_chat")  # 关联数据库
    ...
    @staticmethod # 静态函数方便调用
    def down_reco(user_name):  # 查找用于识别的人脸
        try:
            conn = pymysql.connect(host="121.37.186.99", user="public", password="test", db="python_chat")  # 关联数据库
            cursor = conn.cursor()  # 取得数据库游标
            sql = "SELECT * FROM user_information where user_name = '%s' " % (user_name)  # 数据库语言，按用户名查找
            cursor.execute(sql)   # 游标执行数据库语言
            fout = open('faceReco/识别人脸.png', 'wb')  # 打开存放用户 人脸的路径
